...
export default function LeaderboardContent({ limit = 100 }: LeaderboardContentProps) {
  ...
  const podiumData = [];
  if (silverEntry) {
    podiumData.push({ rank: 2, entry: silverEntry, ...rankConfigs[2] });
  }
  if (goldEntry) {
    podiumData.push({ rank: 1, entry: goldEntry, ...rankConfigs[1] });
  }
  if (bronzeEntry) {
    podiumData.push({ rank: 3, entry: bronzeEntry, ...rankConfigs[3] });
  }
            ...
            {[2, 1, 3].map((rank) => (
              ...
              >
                ...
                <div className="relative -mb-6 z-20 flex items-center justify-center animate-pulse">
                  <Skeleton
                    className={`border-4 border-[#ffffff] shadow-medium rounded-full ${
              ...
              >
                ...
                >
                  <div className="flex flex-col gap-1 w-full mt-auto">
                    <span
                      className={`font-bold tracking-tight ${pilot.textColor} ${
                        pilot.rank === 1 ? "text-xl md:text-2xl" : "text-base md:text-lg"
                      } truncate w-full`}
                    >
                      {truncateAddress(pilot.entry.wallet_address)}
                    </span>
